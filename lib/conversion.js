...
import { readContent, generateFilePath } from '#root/lib/read-write.js';

export async function performConversion(inputFile = '', outputFile = '', format = '') {
  const inputContent = await readContent(inputFile);
  const outputFilePath = generateFilePath(inputFile, outputFile, format);
  console.log(outputFile, format, inputContent);
